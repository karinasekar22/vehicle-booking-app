const { Approval, Booking, Vehicle, Driver } = require("../models");
...
exports.getAllWaitingApproval = async (req, res) => {
  ...
  try {
    const all = await Approval.findAll({
      ...
      include: {
        model: Booking,
        attributes: ["id", "start_date", "end_date", "purpose", "location"],
        include: [
          { model: Vehicle, attributes: ["name"] },
